...
@section('content')
    <div class="row d-flex align-items-center mb-4">
        <div class="col">
            <a class="btn btn-danger" href="{{ url('admin/salidas') }}">
    ...
    <div class="row mb-0">
        <div class="col">
            @if (session('message'))
                <div class="mb-4 alert alert-success alert-dismissible fade show d-flex justify-content-bewteen align-items-center mb-1"
                    role="alert">
                    <div class="col-10">
    ...
    <div class="row">
        <div class="col-12 col-lg-6 col-xl-4 mb-4">
            <div class="card">
                <div class="card-header"><i class="fa-solid fa-people-carry-box"></i> <b>Craer ingreso</b>
                </div>
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ url('admin/ingresos/store') }}">
                        ...
                        <div class="form-floating mb-3">
                            <input type="number" class="form-control" placeholder="Valor" name="cantidad"
                                {{ $errors->has('cantidad') ? 'is-invalid' : '' }} id='cantidadInput' value="" />
                        ...
                        <tbody>
                            @foreach ($inventario as $item)
                                <tr onclick="selectProduct(this);" id="{{ $item->id }}" style="cursor: pointer;">
                                    <td class="text-center">{{ $item->ProductoFk->codigo }}</td>
                                    <td class="text-center">{{ $item->ProductoFk->nombre }}</td>
                                    <td class="text-center">{{ $item->cantidad }}</td>
                                    <td class="text-center">${{ number_format($item->ProductoFk->precio_venta) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
